...
import { generateMess, keepOutBoxes } from '../../src/core/mess.js';
...
for (const [vid, v] of Object.entries(content.villages)) {
  ...
  for (const sid of v.sceneOrder) {
    ...
    for (const tier of [1, 2, 3, 4, 5, 6]) {
      const stat = { village: vid, scene: sid, tier, n: 0, faults: [], short: 0, ambiguous: 0, overlaps: 0, unreachable: 0, underHud: 0, sal: [], times: { novice: [], average: [], skilled: [] }, stamps: { novice: [0, 0, 0], average: [0, 0, 0], skilled: [0, 0, 0] }, types: {} };
      for (let seed = 1; seed <= SEEDS; seed++) {
        ...
        // nothing to find may sit under the play screen's overlays
        const keepOut = keepOutBoxes(content, v.scenes[sid]);
      ...
      const row = {
        ...
        short: stat.short, unreachable: stat.unreachable, ambiguous: stat.ambiguous, overlaps: stat.overlaps, underHud: stat.underHud,
